...
function draw() {
  if(isModelReady) {
    ...
    // if there is at lesat one example in dataset, start classifying
...
function buildButtons() {
  ...
  upButton.mousePressed(function() {
    ups++
    textP2.html("Ups: " + " -Downs:" + downs + " -Lefts: " + lefts + "-Rights: " + rights);
  ...
  leftButton.mousePressed(function() {
    left++
    text2P.html("Ups: " + " -Downs:" + downs + " -Lefts: " + lefts + "-Rights: " + rights);
...
function gotResults(error, results) {
  if(error) {
    ...
    textP2.html("Label: " + results.label);
  }
}
